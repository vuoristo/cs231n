...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        W2 = self.params['W2']
        b2 = self.params['b2']
        linear, cache_linear = affine_forward(X, W1, b1)
        relu, cache_relu = relu_forward(linear)
        scores, cache_scores = affine_forward(relu, W2, b2)
        ...
        dx, dW2, db2 = affine_backward(dloss, cache_scores)
        dx = relu_backward(dx, cache_relu)
        dx, dW1, db1 = affine_backward(dx, cache_linear)
        ...
        loss = 0.5 * self.reg * (np.sum(W1**2) + np.sum(W2**2)) + unregularized_loss
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for l, hidden_dim in enumerate(hidden_dims):
            W = np.random.normal(0.0, weight_scale,
    ...
    def loss(self, X, y=None):
        ...
        regularizer_loss = 0.5 * self.reg * np.sum([np.sum(
        ...
        for l in range(1, self.num_layers)[::-1]:
            ...
            if self.use_dropout:
                ...
                cache = cache[0]
                layer_loss = dropout_backward(layer_loss, dropout_cache)
            if self.use_batchnorm:
